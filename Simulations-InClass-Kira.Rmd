...
```{r}
boot_ci <- function(df) {
  #create an empty df to store stata
  stata_df <- data.frame()
  ...
  #now we need to find percentiles and CI for each column of stata_df
  #df of bounds and CI
  bounds <- data.frame()
  
  for (j in 1:ncol(stata_df)) {
    bounds <- rbind(bounds, quantile(stata_df[,j], c(0.025,0.975)))
  }
